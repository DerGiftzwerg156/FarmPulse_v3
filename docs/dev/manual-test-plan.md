...
| 8.21 | Maintenance repair (T-22) | take a maintenance contract with a worn vehicle (condition < 70 %), wait for the next month | `instructions_ack.json`: `REPAIR_VEHICLE` `APPLIED`; the vehicle shows 0 % damage in the game (and in multiplayer-free single player the workshop shows no repair cost); no *Reparatur* booking appears in the finances |
